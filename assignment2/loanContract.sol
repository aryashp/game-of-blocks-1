...
// You can edit this contract as much as you want. A template is provided here and you can change the function names and implement anything else you want, but the basic tasks mentioned here should be accomplished.
    ...
    function getCompoundInterest(uint256 principle, uint rate, uint time) public pure returns(uint256) {
        ...
        // A good way to prevent overflows will be to typecast principle, rate and the big number divider suggested in the above blogs as uint256 variables, just use 
        uint256 R = rate;
        uint256 T = time;
        uint256 P = principle;
        uint256 precision = 100; // using precision for rate to 2 decimal places
    ...
    function getOwnerBalance() public view returns(uint256) {
	           ...
	           uint256 balance = super.getBalance(Owner);
	           return balance;
    }
    ...
    function viewDues(address creditor) public view isOwner returns(uint256 amt){
        uint256 toPay = loans[creditor];
        return toPay;
    }
    
    function settleDues(address creditor) public isOwner returns(bool){
        uint256 toPay = viewDues(creditor);
        bool flag = super.sendCoin(creditor,toPay,msg.sender);
        if(flag) loans[creditor]=0;
        return flag;
